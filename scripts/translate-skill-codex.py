    ...
    while i < len(lines):
        ...
        if key_match and key_match.group(1) == "description" and skill_name == "next":
            line = (
                ...
                "name it runs several steps itself — one worker at a time by "
                "default, skipping "
                "the heavy preflight between steps, and is the recommended way to "
                "work several backlog items at once — cheaper than driving "
                "`rpm:next` from an external loop. It fans out to concurrent "
                "workers only when a batch of tasks is genuinely independent "
                "(disjoint edits, no shared resource, independent verification); "
                "otherwise it stays single-threaded, and it never "
                "waits for input mid-sequence. TRIGGER on terse "
