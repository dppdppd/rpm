...
if [ -f "$MARKER" ]; then
  ...
  echo "It is the default plan, not a command: the user's first message"
  echo "outranks it. If their opening message asks for something else,"
  echo "switch to it without comment — do not announce the marked task, do"
  echo "not ask whether to switch, do not offer to return to it later. Set"
  echo "the marker's task: field to the new work instead."
  echo ""
  echo "Otherwise: Treat that task as already selected. Do NOT ask whether"
  echo "to continue it, and do NOT offer to switch tasks or wrap up unless"
  echo "the user explicitly asks for that."
  echo "Then:"
  echo "1. Check git state and recent commits to orient."
  echo "2. Create or continue the native task for: ${TASK:-unknown task} — or"
  echo "   for the work the user actually asked for, if they redirected."
  echo "3. Begin working."
  echo ""
  emit_rpm_directives
  exit 0
fi
...
echo "The previous session handed off a specific next step. It is the"
echo "default plan, not a command: the user's first message outranks it."
echo ""
echo "If their opening message asks for something else, drop the handoff"
echo "silently — do not announce it, do not ask whether to switch, do not"
echo "offer to come back to it later. Do what they asked, and set the"
echo "marker's task: field to that work instead."
echo ""
echo "Otherwise — their opening message carries no direction of its own, or"
echo "asks what is next, or says to keep going — then:"
...
echo "2. Treat that handoff as already selected. Do NOT ask whether to"
echo "   continue it, and do NOT offer 'something else' unless the user"
echo "   explicitly rejects this handoff or asks to choose from the backlog."
echo "3. Update the marker task field: Edit docs/rpm/~rpm-session-start —"
echo "   change 'task: (unassigned)' to 'task: $LAST_NEXT'. Preserve"
echo "   session_id and started: fields (the hook already set them)."
