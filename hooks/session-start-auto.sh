...
echo "   any of these to the rpm backlog, or would you rather describe your own?\""
...
echo "   - R  → show ALL tasks from the rpm backlog (including DONE/BLOCKED) with statuses,"
...
echo "Native tasks (TaskCreate / TaskList) = this session's active work only. The rpm backlog (tasks.org) = long-term. Don't mirror backlog adds into the native task list."
echo ""
echo "When you discover a root cause or change approach, lead with \"Key finding:\" so learnings are captured automatically."
echo "When you suggest deferrable work (\"we could X later\", \"worth Y eventually\", \"follow up on Z\"), STOP and ask \"Add to the rpm backlog?\" before moving on — don't just suggest and continue."
echo ""
