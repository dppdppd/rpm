    ...
    while i < len(lines):
        ...
        if key_match and key_match.group(1) == "description" and skill_name == "next":
            line = (
                ...
                "name it runs several steps itself, skipping "
                "the heavy preflight between steps, and is the recommended way to "
                "work several backlog items at once — cheaper than driving "
                "`rpm:next` from an external loop. How wide it fans out — one "
                "worker at a time or a concurrent batch — is the orchestrator's "
                "judgment call, sized to how independent the selected work is; it "
                "never "
