...
## Project Status
- **Current phase**: skills-first migration complete at 2.0.0 (legacy `commands/` deleted). 2.1.x extended `scan.sh` drift detection (`specs_inventory`, `pm_docs_staleness`), added the `/pm:audit quick` tier, closed the session-start drift-catch loop, and fixed the auto-inject + drift-check permission prompts (2.1.2). Next: dogfood 2.1.x against volta.
