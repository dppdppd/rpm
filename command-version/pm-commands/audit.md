...
- empty or unrecognized → print the usage block and stop:
  ...
  - `/pm audit documents` — scan docs + CLAUDE.md + memory + session
    drift via an inline audit pass. Scored findings, hookify
...
jsonl logs for drift. Scored, confidence-gated, hookifies repeat
offenders.

### Phase 1: Scan (inline, no subagent in dispatcher mode)

Run a read-only audit pass inline. Do NOT edit files during this
phase.
...
- Launch `/pm deep-research` in parallel for complex, multi-angle topics.
