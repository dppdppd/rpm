...
- **Direct (single-step)** — no argument, run interactively. May ask one
  ...
  be started safely.
- **Loop** — this invocation is visibly part of `/loop /next`, an
...
- **Sequence** — `/next` was given a count or scope (`N` / `blocked` /
  ...
  Like loop mode it never asks for input mid-sequence; it stops at the
...
1. **Full preflight once, at the start.** Run the delegated preflight
   ...
   ```bash
   bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/log-decision.sh preflight-full "" "sequence start"
   ```
...
`bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/preflight-due.sh` first. The drift scan
...
2. **Decide preflight depth (cadence gate).** Unless this is a Sequence
   ...
   `bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/preflight-due.sh` and branch per
   **Preflight cadence**:
...
3. **Delegated preflight** — when the gate calls for it, dispatch the
   ...
   - `contradiction_script=${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/contradiction-check.sh`
   - `review_ready_script=${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/review-ready.sh`
   - `log_script=${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/log-decision.sh`
...
3. The clear next task is the topmost TODO or IN-PROGRESS entry
   ...
   **Skip entries keyworded `WATCH` (deferred / observe-only) the
   ...
   actionable, call
   `bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/is-pre-completed.sh <id>`;
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/log-decision.sh nudge "" "suggested /next all"
```
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/log-decision.sh <kind> [target] [rationale] [agent-id] [status]
```
...
- `nudge` — empty `<target>`, short `<rationale>` like
  `suggested /next all`. Marks that the one-time Loop mode cheaper-path
...
5. Before finishing, log your result:
   ...
   bash ${CLAUDE_PLUGIN_ROOT}/skills/next/scripts/log-decision.sh backlog-result \
