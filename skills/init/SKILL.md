...
- ADRs required for: new dependencies, API changes, schema changes
```
...
### ADR template (if no ADR directory exists)
...
### Progress tracker (if none exists)

Create `docs/pm/progress/STATUS.md`:
...
### Task tracker (if none exists)

Create `docs/pm/TASKS.org`:
